...
function ns:RepositionTimers()
    if not ns.cdmMode then return end
    ns:UpdateDisplay()
end
...
function ns:InitStandaloneDisplay()
    ...
    iconAnchor:SetScript("OnDragStart", function(self) self:StartMoving() end)
...
function ns:UpdateDisplay()
    ...
    table.sort(buffSlots, function(a, b) return a.spellID < b.spellID end)
